 ...
 * @param language - The language for the analysis result.
 * @returns - Ek object jismein ya to analysis ka data hoga ya error message.
 */
export async function analyzeContent(content: string, language: string) {
  ...
  try {
    ...
    const result = await analyzeContentForCredibility({ content, language });
